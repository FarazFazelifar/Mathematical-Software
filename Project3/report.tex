...
ضرایب چندجمله‌ای (با \lr{np.polyfit}) به دست آمد:
$$
...
\lr{Alg.1} سریع‌ترین الگوریتم است و رشد خیلی ملایمی دارد (از ۵۰ تا ۷۵ میلی‌ثانیه). \lr{Alg.2} زمان متوسطی دارد ولی رشدش یکنواخت و خطی است. \lr{Alg.3} بدترین وضعیت را دارد و از ۱۰۰ به ۶۰۰ میلی‌ثانیه می‌رسد که نشان‌دهنده پیچیدگی بالاتر است.
...
هر سه الگوریتم رفتار خطی دارند ولی شیب‌شان خیلی فرق می‌کند. \lr{Alg.3} شیب تقریباً ۱ دارد (یعنی با دوبرابر شدن داده، زمان هم تقریباً دوبرابر می‌شود). \lr{Alg.1} شیب خیلی کم دارد و عملاً تفاوت چندانی با افزایش داده نمی‌کند.
...
\lr{Alg.1} کمترین پراکندگی (IQR کوچک) و \lr{Alg.3} بیشترین پراکندگی را دارد. هیچ داده پرتی (\lr{outlier}) مشاهده نمی‌شود.


\subsection{ب) افزودن داده جدید}

یک سطر جدید (700KB با مقادیر 80, 320, 700) به فایل اکسل اضافه شد. بعد کد \textbf{بدون هیچ تغییری} دوباره اجرا شد و نمودارها به صورت خودکار آپدیت شدند:
